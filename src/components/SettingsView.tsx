          ...
          <div className="space-y-6 animate-fadeIn">
            <div>
              <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                ...
                <span>Privacidade, Encriptação & Camuflagem</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Definições avançadas de encriptação de ponta a ponta, disfarce de calculadora e proteção contra inspeção.
              </p>
            </div>

            {/* 🧮 MODO CALCULADORA (CAMUFLAGEM DO SMARTPHONE) */}
            <div className="bg-slate-950 p-5 rounded-2xl border border-indigo-500/30 space-y-4 shadow-xl shadow-indigo-500/5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-100 flex items-center gap-2">
                      <span>🧮 Modo Calculadora (Camuflagem do Smartphone)</span>
                    ...
                    <p className="text-xs text-slate-400">
                      Disfarça o PortalTRMobile numa calculadora funcional no telemóvel. Desbloqueia ao digitar o PIN secreto no teclado.
                    </p>
                  </div>
                </div>
                ...
                <button
                  type="button"
                  onClick={handleLockCamouflageNow}
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg shadow-indigo-600/30 transition-all cursor-pointer flex items-center justify-center space-x-2 shrink-0"
                >
                  <Lock className="w-4 h-4" />
                  <span>Bloquear Agora em Calculadora</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-500 font-mono">3 a 8 dígitos</span>
                  </label>
                  <input
                    type="text"
                    pattern="[0-9]*"
                    maxLength={8}
                    value={calcPin}
                    onChange={(e) => handleCalcPinChange(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs font-mono tracking-widest text-white focus:outline-none focus:border-indigo-500"
                    placeholder="Ex: 12345"
                  />
                  <p className="text-[10px] text-slate-500">
                    Ao digitar este PIN no teclado numérico da calculadora, o portal desbloqueia.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300 block">
                    Nome Exibido no Cabeçalho da Calculadora
                  </label>
                  <input
                    type="text"
                    value={calcTitle}
                    onChange={(e) => handleCalcTitleChange(e.target.value)}
                    ...
                    placeholder="Calculadora Padrão"
                  />
                  <p className="text-[10px] text-slate-500">
                    Nome do aplicativo disfarçado exibido na barra superior.
                  </p>
                </div>

                {/* Iniciar Sempre Camouflaged */}
                <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800/80 flex items-center justify-between">
                  <div className="pr-2">
                    <span className="text-xs font-bold text-slate-200 block">Iniciar Sempre em Modo Calculadora</span>
                    <span className="text-[10px] text-slate-400 block">Abrir primeiro a calculadora ao iniciar a aplicação</span>
                  </div>
                  <input
                    type="checkbox"
                    checked={calcStart}
                    onChange={(e) => handleCalcStartChange(e.target.checked)}
                    className="w-5 h-5 accent-indigo-500 rounded cursor-pointer shrink-0"
                  />
                </div>

                {/* Ocultar Botão de Cadeado */}
                <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-800/80 flex items-center justify-between">
                  <div className="pr-2">
                    <span className="text-xs font-bold text-slate-200 block">Ocultar Botão de Cadeado 🔓 (100% Disfarçado)</span>
                    <span className="text-[10px] text-slate-400 block">Remover o ícone visual para disfarce total</span>
                  </div>
                  <input
                    type="checkbox"
                    checked={calcHideBtn}
                    onChange={(e) => handleCalcHideBtnChange(e.target.checked)}
                    className="w-5 h-5 accent-indigo-500 rounded cursor-pointer shrink-0"
                  />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1.5">
                <span className="text-xs font-bold text-slate-200 block">Algoritmo de Encriptação do Barramento</span>
                <select
                  value={settings.dataEncryption}
                  onChange={(e) => updateSetting('dataEncryption', e.target.value as any)}
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2 text-xs text-slate-200 font-bold focus:outline-none focus:border-amber-500 font-mono"
                >
                  <option value="AES-256">AES-256-GCM (Recomendado)</option>
                  <option value="RSA-4096">RSA-4096 / Asymmetric Chaining</option>
                  <option value="NONE">Desativado (Apenas para Testes)</option>
                </select>
              </div>

              {/* Anti-Tracker Camouflage */}
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold text-slate-200 block">Camuflagem Anti-Tracker</span>
                  <span className="text-[11px] text-slate-500 block">Mascarar impressões digitais de rede e cabeçalhos</span>
                </div>
                <input
                  type="checkbox"
                  checked={settings.antiTrackerCamouflage}
                  onChange={(e) => updateSetting('antiTrackerCamouflage', e.target.checked)}
                  className="w-5 h-5 accent-amber-500 rounded cursor-pointer"
                />
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold text-slate-200 block">Guard de Limpeza Remota (Remote Wipe)</span>
                  <span className="text-[11px] text-slate-500 block">Permitir revogação instantânea de chaves e dados em caso de perda</span>
                </div>
                <input
                  type="checkbox"
                  checked={settings.remoteWipeGuard}
                  onChange={(e) => updateSetting('remoteWipeGuard', e.target.checked)}
                  className="w-5 h-5 accent-amber-500 rounded cursor-pointer"
                />
              </div>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center justify-between">
                <div>
                  <span className="text-xs font-bold text-slate-200 block">Enviar Telemetria Anónima</span>
                  <span className="text-[11px] text-slate-500 block">Ajudar a otimizar a estabilidade do motor de eventos</span>
                </div>
                <input
                  type="checkbox"
                  checked={settings.telemetryOptIn}
                  onChange={(e) => updateSetting('telemetryOptIn', e.target.checked)}
                  className="w-5 h-5 accent-amber-500 rounded cursor-pointer"
                />
              </div>

              {/* Expiração de Sessão */}
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 col-span-full">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-200">Limpeza Automática de Sessão (Auto-Clear Cache)</span>
                  <span className="text-xs font-mono font-bold text-amber-400">{settings.autoClearSessionHours} Horas</span>
                </div>
                <input
                  type="range"
                  min={1}
                  max={72}
                  step={1}
                  value={settings.autoClearSessionHours}
                  onChange={(e) => updateSetting('autoClearSessionHours', parseInt(e.target.value))}
                  className="w-full accent-amber-500 bg-slate-800 rounded-lg cursor-pointer"
                />
              </div>
